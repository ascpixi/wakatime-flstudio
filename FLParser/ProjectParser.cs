...
internal class ProjectParser(bool verbose)
{
    ...
    private void ParseEvent(BinaryReader reader)
    {
        try {
            ...
            // This is usually fine, as we're only parsing the project files for statistics
            // and to parse the comment field (which events don't affect)
            Log.Warning("Could not parse an FL Studio event! Ignoring!");
            Log.Warning($"The details about the exception have been printed below.\n\n{ex}\n");
        }
    }
    ...
    private void ParseByteEvent(Enums.Event eventId, BinaryReader reader)
    {
        ...
        if (verbose) OutputLine($"byte: {data:X2}");
    ...
    private void ParseWordEvent(Enums.Event eventId, BinaryReader reader)
    {
        ...
        switch (eventId) {
            ...
            case Enums.Event.WordNewPat:
                while (_project.Patterns.Count < data)
                    _project.Patterns.Add(new Pattern { Id = _project.Patterns.Count });
    ...
    {
        int dataLen = GetBufferLen(reader);
        byte[] dataBytes = reader.ReadBytes(dataLen);
        string unicodeString = Encoding.Unicode.GetString(dataBytes);
        if (unicodeString.EndsWith('\0')) unicodeString = unicodeString[..^1];
        ...
        switch (eventId) {
            ...
            case Enums.Event.TextVersion:
                ...
                if (_project.VersionString.EndsWith("\0"))
                    _project.VersionString = _project.VersionString.Substring(0, _project.VersionString.Length - 1);
    ...
    private void ParseDataEvent(Enums.Event eventId, BinaryReader reader)
    {
        ...
        switch (eventId) {
            ...
            case Enums.Event.DataPatternNotes:
                while (reader.BaseStream.Position < dataEnd) {
                    ...
                    var release = reader.ReadByte(); //It's actually a byte, next byte after is a note channel
                    var color = reader.ReadByte();   //Used for map note color to a midi channel
                ...
                while (reader.BaseStream.Position < dataEnd) {
                    ...
                    switch (messageId) {
                        ...
                        default:
                            if ((int)messageId >= 64 &&
                                (int)messageId <=
                                64 + 104)  // any value 64 or above appears to be the desination insert
                            {
                                var insertDest = (int)messageId - 64;
                                insert.RouteVolumes[insertDest] = messageData;
                                OutputLine(
                                    $"{startPos:X4} insert send from {insertId} to {insertDest} volume: {messageData:X8}");
                            }
                            else {
                                OutputLine(
                                    $"{startPos:X4} insert param: {messageId} {insertId}-{slotId}, data: {messageData:X8}");
                            }
            ...
            case Enums.Event.DataAutomationChannels:
                while (reader.BaseStream.Position < dataEnd) {
                    ...
                    if ((paramDestination & 0x2000) == 0 &&
                        paramDestination < _project.Channels.Count)  // Automation on channel
                    {
                        channel.Data = new AutomationData {
                            ...
                            VstParameter =
                                (param & 0x8000) > 0
                                    ? true
                                    : false      // switch determines if automation is on channel or vst
                        };
                    }
                    else {
                        channel.Data = new AutomationData // automation on insert slot
            ...
            case Enums.Event.DataPlayListItems:
                while (reader.BaseStream.Position < dataEnd) {
                    ...
                    // This is a workaround for newer project data not loading - this is
                    // fine for now, as we only parse projects for progress estimates.
                    // TODO: Fix this for newer FL Studio versions
                    if (track is < 0 or >= 500)
                        continue;

                    if (patternId >= _project.Channels.Count)
                        continue;
                    ...
                    // id of 0-patternBase is samples or automation, after is a pattern
                    if (patternId <= patternBase) {
                        var startOffset = (int)(reader.ReadSingle() * _project.Ppq);
                        var endOffset = (int)(reader.ReadSingle() * _project.Ppq);
                        ...
                        if ((patternId - patternBase - 1) > _project.Patterns.Count) {
                            Console.WriteLine("Pattern found on track " + (track) +
                                              ", but it seems to be empty. Skipping...");
                            break;
                        }
        ...
        // make sure the cursor is at the end of data
    ...
    private Plugin? ParsePluginChunk(byte[] chunk)
    {
        ...
        while (reader.BaseStream.Position < reader.BaseStream.Length) {
            ...
            switch (eventId) {
                ...
                default:
                    OutputLine(
                        $"Event {eventId}, data: {string.Join(" ", reader.ReadBytes(length).Select(x => x.ToString("X2")))}");
                    break;
            }
        }
